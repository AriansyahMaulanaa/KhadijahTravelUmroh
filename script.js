...
document.addEventListener("DOMContentLoaded", function() {
    ...
    function renderPackages(filter = '') {
      ...
      container.innerHTML = filtered.map(pkg => `
        <div class="bg-white rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-300 overflow-hidden card-hover group">
          <div class="relative overflow-hidden">
            ...
            <div class="absolute top-4 left-4 bg-white bg-opacity-90 backdrop-blur-sm px-2 py-1 rounded-lg">
              <i data-lucide="star" class="w-4 h-4 text-yellow-400 fill-current inline"></i>
          ...
          <div class="p-6">
            ...
            <button onclick="openPackageModal(${pkg.id})" 
                    class="w-full bg-gradient-to-r from-blue-500 to-blue-600 text-white py-3 rounded-xl font-semibold hover:from-blue-600 hover:to-blue-700 transition-all duration-300 flex items-center justify-center">
              Detail Paket <i data-lucide="chevron-right" class="w-4 h-4 ml-2"></i>
            </button>
          </div>
        </div>
      `).join('');
    ...
    function openPackageModal(id) {
      ...
      const includes = pkg.includes.map(item => 
        `<div class="flex items-center"><div class="w-2 h-2 bg-blue-500 rounded-full mr-2"></div><span class="text-gray-700">${item}</span></div>`
      ).join('');
      document.getElementById('modal-includes').innerHTML = includes;
